...
function mapTuple(tuple, fields, callback) {
	return assertedValue("tuple", tuple).items.reduce(function (result, item, i) {
		if (fields[i] != null) {
			var value = void 0;
...
function parseDerivation(text) {
	var root = parseAST(text);

	return {
		outputs: mapArray(root.items[0], function (item) {
			return mapTuple(item, ["name", "path", "expectedHashAlgorithm", "expectedHash"]);
		}),
		inputDerivations: mapArray(root.items[1], function (item) {
			return mapTuple(item, ["path", "wantedOutputs"], function (value, i) {
				if (i === 1) {
					/* An `array` of outputs */
					return value.items;
				} else {
		...
		environmentVariables: mapArray(root.items[6], function (item) {
